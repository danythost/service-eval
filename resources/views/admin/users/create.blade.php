    ...
    <div class="max-w-2xl">
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700 overflow-hidden">
            <div class="px-6 py-4 border-b border-gray-100 dark:border-gray-700 bg-gray-50 dark:bg-gray-900/50">
                <h3 class="text-xl font-bold text-gray-800 dark:text-white">Create New User</h3>
                <p class="text-sm text-gray-500 mt-1">Register a new system administrator or service provider.</p>
            </div>
            ...
            <form action="{{ route('admin.users.store') }}" method="POST" class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            @foreach($roles as $role)
                                <label class="relative flex items-center p-4 border rounded-xl cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-all {{ old('role') === $role->value ? 'border-blue-500 ring-1 ring-blue-500 bg-blue-50 dark:bg-blue-900/10' : 'border-gray-200 dark:border-gray-700' }}">
                                    <input type="radio" name="role" value="{{ $role->value }}" class="sr-only" {{ old('role') === $role->value ? 'checked' : '' }}>
                                    <div class="flex-1">
                                    ...
                                    <div class="ml-auto w-5 h-5 border-2 rounded-full border-gray-300 flex items-center justify-center {{ old('role') === $role->value ? 'border-blue-500 bg-blue-500 shadow-lg' : '' }}">
                ...
                <div class="pt-6 border-t border-gray-100 dark:border-gray-700 flex items-center justify-end space-x-4">
                    <a href="{{ route('admin.users.index') }}" class="text-sm font-bold text-gray-500 hover:text-gray-700 dark:hover:text-gray-300 transition-colors">Cancel Listing</a>
        ...
        document.querySelectorAll('input[name="role"]').forEach(radio => {
            radio.addEventListener('change', function() {
                document.querySelectorAll('input[name="role"]').forEach(r => {
                    r.closest('label').classList.remove('border-blue-500', 'ring-1', 'ring-blue-500', 'bg-blue-50', 'dark:bg-blue-900/10');
                    r.closest('label').classList.add('border-gray-200', 'dark:border-gray-700');
                    const check = r.closest('label').querySelector('.ml-auto div') || r.closest('label').querySelector('.ml-auto');
